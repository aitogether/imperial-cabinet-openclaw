...
上面的三步上手是给已经熟悉 OpenClaw 的用户看的。如果你是第一次接触，下面是一个更友好的新手引导：

### 典型任务：帮父母预约挂号

假设你想帮住在老家的父母预约三甲医院的体检，流程是这样的：

1. **皇帝下旨**：你在 OpenClaw 工作区创建一个 issue，标题写"为父母预约XX医院体检"，正文附上父母医保卡信息、期望时间、医院科室偏好。
2. **首辅承旨**：御前首辅自动接手，把任务拆解：
   - 户部查预算（体检费用、是否走医保）
   - 工部查路线（医院地理位置、交通方式）
   - 礼部拟话术（打电话预约的话术模板）
   - 刑部过合规（个人信息使用授权）
3. **六部分理**：各部并行处理，工部同时查3家备选医院，礼部准备2套话术，户部确认支付方式。
4. **御前呈报**：所有材料汇总到皇帝这里，你只需要看一眼，点"批准"，OpenClaw 自动打电话/在线预约，把确认短信转发给你。
5. **东厂存档**：整个过程记录在案，下次类似任务可以直接复用模板。

整个过程你只写了1条 issue、点了1次"批准"，AI Agents 完成了十几步操作。

---

> **想试试？** 先从 [examples/appointment-booking/](examples/appointment-booking/) 示例文件夹开始，里面有完整的 issue 模板和各部门的 Markdown 配置文件。
